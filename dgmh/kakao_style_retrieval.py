...
``~/.hermes/dgmh/kakao_hako_corpus.txt`` (private, kept outside the
...
_DEFAULT_CORPUS_PATH = Path.home() / ".hermes" / "dgmh" / "kakao_hako_corpus.txt"
...
def _get_corpus(corpus_path: Optional[Path] = None) -> _StyleCorpus:
    ...
    path = corpus_path or Path(
        os.environ.get("DGMH_KAKAO_CORPUS_PATH", "") or _DEFAULT_CORPUS_PATH
    )
    with _corpus_lock:
...
_PATINA_PROFILES_DIR = (
    Path.home() / ".hermes" / "skills" / "creative" / "patina" / "profiles"
)
...
def rewrite_with_rag_profile(
    ...
    *,
    ...
    profile_dir = profiles_dir or _PATINA_PROFILES_DIR
