...
_FORCE_BANMAL_FIXES = (
    ...
    (re.compile(r"주세요(?P<s>[.!?…~ㅋㅎㅠㅜ]*)(?=$|\s)"), r"줘\g<s>"),
    ...
    (re.compile(r"아니요(?P<s>[.!?…~ㅋㅎㅠㅜ]*)(?=$|\s)"), r"아니\g<s>"),
    # 1인칭 격식 → 반말 (word-boundary lookbehind so 저녁/저것/제목 안 잡음)
    (re.compile(r"(?<![가-힣])저는(?=[\s,.!?])"), r"난"),
    (re.compile(r"(?<![가-힣])저한테(?=[\s,.!?])"), r"나한테"),
    (re.compile(r"(?<![가-힣])저도(?=[\s,.!?])"), r"나도"),
    (re.compile(r"(?<![가-힣])저를(?=[\s,.!?])"), r"날"),
    (re.compile(r"(?<![가-힣])저가(?=[\s,.!?])"), r"내가"),
    (re.compile(r"(?<![가-힣])제가(?=[\s,.!?])"), r"내가"),
    (re.compile(r"(?<![가-힣])저희가(?=[\s,.!?])"), r"우리가"),
    (re.compile(r"(?<![가-힣])저희(?=[\s,.!?])"), r"우리"),
    ...
    # -래요 / -ㄹ래요 (그래요, 갈래요, 살래요, 볼래요)
    (re.compile(r"래요(?P<s>[.!?…~ㅋㅎㅠㅜ]*)(?=$|\s)"), r"래\g<s>"),
)
