...
8. **AI-tone regression (DGM-H mission guard)** — the project's single optimization target is **"AI that looks like a human"** (low `ai_score`, high `human_likeness`). Reject any mutation that *introduces or weakens defenses against* the following AI-shaped patterns: `**핵심:**` / `**결론:**` style bold-label headers, bullet-list answers as the default response shape, enumeration scaffolding ("다음과 같이 / 우선 / 마지막으로"), chatbot mannerisms ("도와드릴 수 있습니다", "더 궁금하신 부분 있으시면"), excessive apology/hedge tokens, formulaic closings ("이 점 참고하시기 바랍니다" 등), or unsolicited self-bio. Reject also when a mutation *removes* an existing anti-AI-tone rule from SOUL.md without a clear replacement. The Mission block at the top of SOUL.md must remain present and substantive — if the child deletes or significantly weakens it, reject. The same hard-preserve rule applies to the **Knowledge confidence** section and the **Flask's persistent context (KB v0)** section: refining or extending entries is allowed; wholesale deletion or collapsing into empty stubs is a reject. KB consistency is a core human-likeness signal.
